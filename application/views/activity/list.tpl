<div class="bjui-pageHeader">
    <a class="btn btn-default" href="operation/activity/set" data-toggle="navtab"  data-id="notice-add" >新增活动</a>
    <form id="pagerForm" class="frm_ujbinsure" data-toggle="ajaxsearch" action="operation/activity/lists" method="post">
...
<div class="bjui-pageContent tableContent">
    <table class="table table-bordered table-hover table-striped table-top">
        ...
        <tr data-id="{{$key}}">
            <td>{{$v['row_id']}}</td>
            <td>{{$v['name']}}</td>
            <td><image src="{{$v['image']}}" height="40"/></td>
            ...
            <td>
                {{if $v['status'] == 1}}
                <a href="operation/activity/line?id={{$v['id']}}&status=2" class="btn btn-red" data-toggle="doajax">下架</a>
                {{else}}
                <a href="operation/activity/line?id={{$v['id']}}&status=1" class="btn btn-blue" data-toggle="doajax">上架</a>
                {{/if}}
                <!--<a href="operation/activity/set?id={{$v['id']}}" class="btn btn-green" data-toggle="dialog" data-id="form" data-width="600" data-height="400" >编辑</a>-->
            </td>
        </tr>
        {{/foreach}}
        </tbody>
