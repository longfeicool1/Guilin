<div class="bjui-pageHeader" style="marign:5px;">
    <form id="pagerForm" class="frm_weekList" data-toggle="ajaxsearch" action="/report/report/weekList" method="post">
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-toggle="tablefixed" data-width="100%" data-nowrap="true">
        ...
        <tbody>
            ...
            <tr data-id="{{$v['id']}}">
                ...
                <td rowspan="3">{{$v['score_title']}}</td>
                <td rowspan="3">{{$v['week_income']}}</td>
                <td>{{$v['week_acce']}}</td>
                <td>{{$v['week_dece']}}</td>
                <td>{{$v['week_coce']}}</td>
                <td>{{$v['trip_mile']}}</td>
                <td>{{$v['trip_time']}}</td>
                <td>{{$v['top_speed']}}</td>
                <td rowspan="3">{{$v['getIncomeStatus']}}</td>
            </tr>
            <tr>
...
<script>
    $('.frm_weekList select').change(function (){
        $('.frm_weekList').submit();
    })

    // function daybaoExport(){
